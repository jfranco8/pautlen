%{
  ...
  int es_funcion = 0;
  int _return = 0; // variable que nos indica si la función tiene retorno o no
  int return_type;
...
asignacion: TOK_IDENTIFICADOR TOK_ASIGNACION exp {
                  ...
                  if (is_global_symbol(ts_get_global(ts), $1.lexema) == NULL){
                    if(get_symbol_category(simbolo) == PARAMETRO){
                      escribirVariableLocal(out, get_posicion_var_local(simbolo));
          ...
          | elemento_vector TOK_ASIGNACION exp {
            ...
            escribir_elemento_vector(out, $1.lexema, get_len(simbolo), $3.es_direccion);
            asignarDestinoEnPila(out, $3.es_direccion);
...
lectura: TOK_SCANF TOK_IDENTIFICADOR {
          ...
          else if(get_symbol_category(simbolo) == FUNCION){
            fprintf(out,"****Error semantico en lin %d: Asignacion incompatible.\n", linea);
            return -1;
          }
          else if (get_category(simbolo) == VECTOR){
...
exp: exp TOK_MAS exp {
   ...
   | TOK_IDENTIFICADOR {
     ...
     if (is_global_symbol(ts_get_global(ts), $1.lexema) == NULL){
       if(get_symbol_category(simbolo) == PARAMETRO){
         escribirParametro(out, get_num_param(simbolo), num_parametros_actual);
       }else{
         escribirVariableLocal(out, get_posicion_var_local(simbolo));
       }
     } else{
   ...
   | idf_llamada_funcion TOK_PARENTESISIZQUIERDO lista_expresiones TOK_PARENTESISDERECHO
     {/*Control de errores de simbolo*/
       ...
       llamada = 0;
       $$.tipo = get_type(simbolo);
       llamarFuncion(out, $1.lexema, get_num_param(simbolo));
      };
